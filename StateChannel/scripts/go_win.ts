...
async function main() {
    ...
    const game = await StateChannel.deploy(p2.address, challengeLength, p1Commitment, boardsize, { value: bet });
    ...
    const currentPlayer = await game.currentPlayerBit();
    ...
    const challengeTx = await game.connect(currentPlayer).challenge(currentPlayerVal, encodedPrevState, signedPrevState, encodedCaptureScores, {gasLimit: gasLimit});
