...
describe("StateChannel contract", function() {
    async function deployStateChannelFixture() {
        const turnLength = 10;
        const p1Nonce = 101;
        const p2Nonce = 42;
        ...
        const game = await StateChannel.deploy(p2.address, turnLength, p1Commitment, { value: bet });
    ...
    it("joinGame: only p2 can join if stake matched", async function() {
        const { game, p2, p3, bet, p2Nonce } = await loadFixture(deployStateChannelFixture);
    ...
    it("startGame: p1Nonce must match p1Commitment", async function() {
        const { game, p1, p2, bet, p1Nonce, p2Nonce, firstMover } = await loadFixture(deployStateChannelFixture);
        await expect(game.connect(p2).joinGame(p2Nonce, {value: bet })).not.to.be.reverted;
        ...
        expect(await game.currentPlayerBit()).to.equal(firstMover);
    })

    it ("challenge: firstMover wins", async function() {
        const { game, p1, p2, bet, p1Nonce, p2Nonce, bitToSqVal, firstMover, bitToPlayer } = await loadFixture(deployStateChannelFixture);
        await expect(game.connect(p2).joinGame(p2Nonce, {value: bet })).not.to.be.reverted;
        await expect(game.connect(p1).startGame(p1Nonce)).not.to.be.reverted;
        ...
        await expect(game.connect(currentPlayer).challenge(currentPlayerVal, encodedPrevState, signedPrevState, encodedWinningMoves)).not.to.be.reverted;
